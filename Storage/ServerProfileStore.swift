...
/// Keychain-backed store for `ServerProfile`s.
/// Each profile is one keychain item under service `ai.opencode.client.ios`,
/// account `profile.<uuid>`, with the JSON-encoded profile as the password blob.
/// Active profile ID is stored separately in `AppPreferences` (it isn't sensitive).
final class ServerProfileStore: Sendable {
    private static let service = "ai.opencode.client.ios"
